...
def generate_siren_wav(output_file='siren.wav', duration=5.0, sample_rate=44100, volume=0.7,
    ...
    Siren types:
    - 'wail': US-style continuous frequency sweep (more realistic, less spacey)
    ...
    if siren_type == 'wail':
        ...
        # More realistic frequencies - start low, sweep higher
        base_freq_start = 500  # Hz
        base_freq_end = 900  # Hz
        
        # Frequency sweep modulation - less smooth, more urgent
        modulation = 0.5 * (1 + np.sin(2 * np.pi * siren_speed * t))
        
        # Add slight randomness to sweep for realism
        sweep_noise = 1.0 + 0.1 * np.sin(2 * np.pi * 15 * t)
        
        # Apply Doppler shift to frequencies
        freq_start = calculate_doppler_shift(base_freq_start, radial_velocity)
        freq_end = calculate_doppler_shift(base_freq_end, radial_velocity)
        
        # Interpolate between frequencies with slight randomness
        freq = freq_start + (freq_end - freq_start) * modulation * sweep_noise
        
        # Generate phase-locked oscillator
        phase = 2 * np.pi * np.cumsum(freq) / sample_rate
        signal = np.sin(phase) * 0.5
        
        # Add more harmonics for grit/harshness (odd harmonics characteristic of mechanical sirens)
        signal += 0.3 * np.sin(3 * phase)  # 3rd harmonic
        signal += 0.15 * np.sin(5 * phase)  # 5th harmonic
        signal += 0.1 * np.sin(7 * phase)  # 7th harmonic
        
        # Add slight noise for texture
        noise = np.random.normal(0, 0.05, len(t))
        signal += noise
        
        # Add slight tremolo (amplitude modulation) like real sirens
        ...
        signal_hi += 0.2 * np.sin(3 * phase_hi)
        signal_lo += 0.2 * np.sin(3 * phase_lo)
        ...
        yelp_freq = 700  # Hz - higher pitched
    ...
    # Envelope - punchy attack, no decay (continuous siren)
    attack_time = 0.02  # 20ms punchy attack
    ...
    # Less aggressive filtering - keep more high frequency for realism
    # Only soft filter very high harmonics
    lp_filter_size = int(sample_rate / 6000)  # Cutoff ~6kHz (higher than before)
...
if __name__ == "__main__":
    ...
    generate_siren_wav('siren_us_wail_realistic.wav', 
                       siren_type='wail', 
                       apply_doppler=True,
